"""Assembly tools — add component, mate, list components, reposition."""

from __future__ import annotations

from typing import Any

from nxpilot_ai.nx_session import NXSession
from nxpilot_ai.response import ToolError, ToolResult
from nxpilot_ai.tools.registry import mcp_tool

# ---------------------------------------------------------------------------
# Mate type mapping
# ---------------------------------------------------------------------------
_MATE_TYPES = {
    "touch": "Touch",
    "align": "Align",
    "orient": "Orient",
    "center": "Center",
    "align_angle": "AlignAngle",
}


# ---------------------------------------------------------------------------
# 1. nx_add_component
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_add_component",
    description="Add a component from a .prt file to the current assembly.",
    params={
        "part_path": {
            "type": "string",
            "description": "Full file path of the .prt part file to add as a component.",
            "required": True,
        },
        "name": {
            "type": "string",
            "description": "Optional display name for the new component.",
            "required": False,
        },
    },
)
async def nx_add_component(
    part_path: str,
    name: str | None = None,
) -> ToolResult | ToolError:
    """Add a component from a .prt file into the current assembly."""
    try:
        work_part = NXSession.get_instance().require_work_part()

        comp_assembly = work_part.ComponentAssembly
        component = comp_assembly.AddComponent(part_path, name or "")

        comp_name = component.Name() if callable(getattr(component, "Name", None)) else getattr(component, "Name", name or part_path)

        return ToolResult.success(
            data={"component": comp_name, "part_path": part_path},
            message=f"Added component '{comp_name}' from {part_path}.",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 2. nx_mate_component
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_mate_component",
    description="Apply a mating constraint between assembly components.",
    params={
        "component": {
            "type": "string",
            "description": "Name of the component to apply the mate to.",
            "required": True,
        },
        "mate_type": {
            "type": "string",
            "description": "Type of mate: touch, align, orient, center, align_angle.",
            "required": True,
        },
        "references": {
            "type": "array",
            "description": "List of reference names (edges, faces, datums) for the mate.",
            "required": False,
        },
        "offset": {
            "type": "number",
            "description": "Offset distance for the mate constraint. Default is 0.",
            "required": False,
        },
    },
)
async def nx_mate_component(
    component: str,
    mate_type: str,
    references: list[str] | None = None,
    offset: float = 0.0,
) -> ToolResult | ToolError:
    """Apply a mating constraint between assembly components."""
    try:
        import NXOpen

        work_part = NXSession.get_instance().require_work_part()

        mate_key = mate_type.strip().lower()
        if mate_key not in _MATE_TYPES:
            valid = ", ".join(_MATE_TYPES.keys())
            return ToolError(
                error_code="NX_INVALID_PARAMS",
                message=f"Unsupported mate type: '{mate_type}'.",
                suggestion=f"Use one of: {valid}.",
            )

        comp_assembly = work_part.ComponentAssembly
        constraints_builder = comp_assembly.CreateConstraintsBuilder()

        type_map = {
            "touch": NXOpen.Assemblies.Constraint.Type.Touch,
            "align": NXOpen.Assemblies.Constraint.Type.Align,
            "orient": NXOpen.Assemblies.Constraint.Type.Orient,
            "center": NXOpen.Assemblies.Constraint.Type.Center,
            "align_angle": NXOpen.Assemblies.Constraint.Type.Angular,
        }
        constraint_type = type_map[mate_key]

        constraint = constraints_builder.CreateConstraint(constraint_type)
        constraint.Component = component

        if offset != 0.0:
            constraint.Offset = offset

        if references:
            for i, ref in enumerate(references):
                if i == 0:
                    constraint.FirstConstraintReference = ref
                elif i == 1:
                    constraint.SecondConstraintReference = ref

        constraints_builder.Commit()
        constraints_builder.Destroy()

        result_data: dict[str, Any] = {
            "component": component,
            "mate_type": mate_type,
            "offset": offset,
        }
        if references:
            result_data["references"] = references

        return ToolResult.success(
            data=result_data,
            message=f"Applied '{mate_type}' mate to component '{component}'.",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 3. nx_list_components
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_list_components",
    description="List all components in the current assembly.",
    params={},
)
async def nx_list_components() -> ToolResult | ToolError:
    """List all components in the current work part's assembly."""
    try:
        work_part = NXSession.get_instance().require_work_part()

        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        components_list: list[dict[str, Any]] = []

        if root_component is not None:
            children = root_component.GetChildren()
            for child in children:
                child_name = child.Name() if callable(getattr(child, "Name", None)) else getattr(child, "Name", "unknown")
                child_data: dict[str, Any] = {"name": str(child_name)}
                components_list.append(child_data)

        return ToolResult.success(
            data={"components": components_list, "count": len(components_list)},
            message=f"Assembly contains {len(components_list)} component(s).",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 4. nx_reposition_component
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_reposition_component",
    description="Move a component by translation and rotation offsets.",
    params={
        "component": {
            "type": "string",
            "description": "Name of the component to reposition.",
            "required": True,
        },
        "dx": {
            "type": "number",
            "description": "Translation offset along X axis. Default is 0.",
            "required": False,
        },
        "dy": {
            "type": "number",
            "description": "Translation offset along Y axis. Default is 0.",
            "required": False,
        },
        "dz": {
            "type": "number",
            "description": "Translation offset along Z axis. Default is 0.",
            "required": False,
        },
        "rx": {
            "type": "number",
            "description": "Rotation offset around X axis in degrees. Default is 0.",
            "required": False,
        },
        "ry": {
            "type": "number",
            "description": "Rotation offset around Y axis in degrees. Default is 0.",
            "required": False,
        },
        "rz": {
            "type": "number",
            "description": "Rotation offset around Z axis in degrees. Default is 0.",
            "required": False,
        },
    },
)
async def nx_reposition_component(
    component: str,
    dx: float = 0.0,
    dy: float = 0.0,
    dz: float = 0.0,
    rx: float = 0.0,
    ry: float = 0.0,
    rz: float = 0.0,
) -> ToolResult | ToolError:
    """Reposition a component by translation and rotation offsets."""
    try:
        import math

        import NXOpen

        work_part = NXSession.get_instance().require_work_part()

        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent
        target = None
        if root_component is not None:
            for child in root_component.GetChildren():
                child_name = child.Name() if callable(getattr(child, "Name", None)) else getattr(child, "Name", "")
                if child_name == component:
                    target = child
                    break

        if target is None:
            return ToolError(
                error_code="NX_NOT_FOUND",
                message=f"Component '{component}' not found in assembly.",
                suggestion="Use nx_list_components to see available components.",
            )

        transform = NXOpen.Transform()
        transform.Translation = NXOpen.Vector3d(dx, dy, dz)

        cx, sx = math.cos(math.radians(rx)), math.sin(math.radians(rx))
        cy, sy = math.cos(math.radians(ry)), math.sin(math.radians(ry))
        cz, sz = math.cos(math.radians(rz)), math.sin(math.radians(rz))

        rot = NXOpen.Matrix3x3()
        rot.Xx = cy * cz
        rot.Xy = sx * sy * cz - cx * sz
        rot.Xz = cx * sy * cz + sx * sz
        rot.Yx = cy * sz
        rot.Yy = sx * sy * sz + cx * cz
        rot.Yz = cx * sy * sz - sx * cz
        rot.Zx = -sy
        rot.Zy = sx * cy
        rot.Zz = cx * cy
        transform.Rotation = rot

        comp_assembly.MoveComponent(target, transform)

        return ToolResult.success(
            data={
                "component": component,
                "translation": [dx, dy, dz],
                "rotation": [rx, ry, rz],
            },
            message=f"Repositioned component '{component}' "
                    f"(dx={dx}, dy={dy}, dz={dz}, rx={rx}, ry={ry}, rz={rz}).",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 5. nx_generate_bom
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_generate_bom",
    description="Generate a Bill of Materials (BOM) from the current assembly.",
    params={
        "format": {
            "type": "string",
            "description": "Output format (text, json). Default is json.",
            "required": False,
        },
    },
)
async def nx_generate_bom(format: str = "json") -> ToolResult | ToolError:
    """Generate a simple Bill of Materials from the assembly."""
    try:
        work_part = NXSession.get_instance().require_work_part()

        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        bom_items: list[dict[str, Any]] = []
        part_counts: dict[str, int] = {}

        def traverse_component(comp, level=0):
            if comp is None:
                return
            comp_name = comp.Name() if callable(getattr(comp, "Name", None)) else getattr(comp, "Name", "unknown")
            part_path = getattr(comp, "Path", "")
            key = str(comp_name)
            if key in part_counts:
                part_counts[key] += 1
            else:
                part_counts[key] = 1
                bom_items.append({
                    "name": comp_name,
                    "level": level,
                    "path": part_path,
                })
            for child in comp.GetChildren():
                traverse_component(child, level + 1)

        if root_component is not None:
            traverse_component(root_component)

        # Add quantities to bom items
        for item in bom_items:
            item["quantity"] = part_counts[item["name"]]

        if format.lower() == "text":
            lines = ["Bill of Materials", "=" * 40]
            for item in bom_items:
                lines.append(f"{item['quantity']:3d} x {item['name']} (Level {item['level']})")
            bom_text = "\n".join(lines)
            return ToolResult.success(
                data={"bom": bom_items, "count": len(bom_items)},
                message=bom_text,
            )
        else:  # json
            return ToolResult.success(
                data={"bom": bom_items, "count": len(bom_items)},
                message=f"Generated BOM with {len(bom_items)} unique part(s), total {sum(part_counts.values())} component(s).",
            )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 6. nx_find_duplicate_parts
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_find_duplicate_parts",
    description="Find duplicate components in the current assembly.",
    params={},
)
async def nx_find_duplicate_parts() -> ToolResult | ToolError:
    try:
        work_part = NXSession.get_instance().require_work_part()

        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        part_paths = {}
        duplicates = []

        def traverse(comp):
            if comp is None:
                return
            comp_name = getattr(comp, "Name", lambda: "unknown")() if callable(getattr(comp, "Name", None)) else getattr(comp, "Name", "unknown")
            comp_path = getattr(comp, "Path", "")
            
            key = f"{comp_name}:{comp_path}"
            if key in part_paths:
                duplicates.append({
                    "name": comp_name,
                    "path": comp_path,
                    "instances": [part_paths[key], comp]
                })
            else:
                part_paths[key] = comp
            
            for child in getattr(comp, "GetChildren", lambda: [])():
                traverse(child)
        
        if root_component is not None:
            traverse(root_component)
        
        return ToolResult.success(
            data={"duplicates": duplicates, "count": len(duplicates)},
            message=f"Found {len(duplicates)} duplicate part(s) in the assembly." if duplicates else "No duplicate parts found.",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 7. nx_list_materials
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_list_materials",
    description="List all materials used in the assembly.",
    params={},
)
async def nx_list_materials() -> ToolResult | ToolError:
    try:
        work_part = NXSession.get_instance().require_work_part()
        
        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        materials = {}

        def traverse(comp):
            if comp is None:
                return
            comp_name = getattr(comp, "Name", lambda: "unknown")() if callable(getattr(comp, "Name", None)) else getattr(comp, "Name", "unknown")
            
            # In real implementation, query NXOpen for material property
            material = "Steel"  # Default placeholder
            if material not in materials:
                materials[material] = []
            materials[material].append(comp_name)
            
            for child in getattr(comp, "GetChildren", lambda: [])():
                traverse(child)
        
        if root_component is not None:
            traverse(root_component)
        
        return ToolResult.success(
            data={"materials": materials, "count": len(materials)},
            message=f"Found {len(materials)} material(s) used in the assembly." if materials else "No materials listed (implementation placeholder).",
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 8. nx_count_fasteners
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_count_fasteners",
    description="Count fasteners (bolts, screws, nuts, washers) in the assembly.",
    params={},
)
async def nx_count_fasteners() -> ToolResult | ToolError:
    try:
        work_part = NXSession.get_instance().require_work_part()
        
        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        fastener_types = ["bolt", "screw", "nut", "washer"]
        fasteners = {t: 0 for t in fastener_types}

        def traverse(comp):
            if comp is None:
                return
            comp_name = (getattr(comp, "Name", lambda: "unknown")() if callable(getattr(comp, "Name", None)) else getattr(comp, "Name", "unknown")).lower()
            for ft in fastener_types:
                if ft in comp_name:
                    fasteners[ft] += 1
            for child in getattr(comp, "GetChildren", lambda: [])():
                traverse(child)
        
        if root_component is not None:
            traverse(root_component)
        
        total = sum(fasteners.values())
        
        return ToolResult.success(
            data={"fasteners": fasteners, "total": total},
            message=f"Found {total} fasteners in total: {', '.join([f'{k}: {v}' for k, v in fasteners.items()])}." if total else "No fasteners found (search is based on component names)."
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 9. nx_detect_interference
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_detect_interference",
    description="Detect collisions and interferences between assembly components.",
    params={},
)
async def nx_detect_interference() -> ToolResult | ToolError:
    try:
        work_part = NXSession.get_instance().require_work_part()
        
        # Skeleton: in real implementation, use NXOpen CAE or Assembly Interference Check API
        interferences = []
        
        return ToolResult.success(
            data={"interferences": interferences, "count": len(interferences)},
            message="Interference detection complete (placeholder - real implementation would use NXOpen Assembly Interference Check API)."
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)


# ---------------------------------------------------------------------------
# 10. nx_find_missing_components
# ---------------------------------------------------------------------------
@mcp_tool(
    name="nx_find_missing_components",
    description="Find missing/unloaded components in the assembly.",
    params={},
)
async def nx_find_missing_components() -> ToolResult | ToolError:
    try:
        work_part = NXSession.get_instance().require_work_part()
        
        comp_assembly = work_part.ComponentAssembly
        root_component = comp_assembly.RootComponent

        missing = []

        def traverse(comp):
            if comp is None:
                return
            # In real NXOpen, check component status
            # For now, just return placeholder
            for child in getattr(comp, "GetChildren", lambda: [])():
                traverse(child)
        
        if root_component is not None:
            traverse(root_component)
        
        return ToolResult.success(
            data={"missing": missing, "count": len(missing)},
            message="Missing components check complete (placeholder - real implementation checks NX component load status)."
        )

    except Exception as exc:
        return ToolResult.from_exception(exc)
